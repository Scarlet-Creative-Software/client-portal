        ...
        $actions['index']['prepareDataProvider'] = function () {
            ...
            if (isset($q)) {
                ...
                if (!empty($q)) {
                    ...
                    } elseif (preg_match('/(status|STATUS|submission_status|SUBMISSION_STATUS):(verified|VERIFIED|confirmed|CONFIRMED)/', $q, $matches)) { // Filter by Submission Status
                        if (!empty($matches[2])) {
                            $status = 7;
                            $query = FormSubmission::find()
                                ->where('form_id=:form_id', [':form_id' => $id])
                    ...
                    } elseif (preg_match('/(status|STATUS|submission_status|SUBMISSION_STATUS):(unverified|UNVERIFIED|unconfirmed|UNCONFIRMED)/', $q, $matches)) { // Filter by Submission Status
                        if (!empty($matches[2])) {
                            $status = 1;
                            $query = FormSubmission::find()
                                ->where('form_id=:form_id', [':form_id' => $id])
